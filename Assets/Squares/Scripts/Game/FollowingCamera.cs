...
public class FollowingCamera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        // 距離は本ゲームでは固定なのでエディタ未再生の時だけ調整可能に
        if(!Application.isPlaying)
        {
            updateDistance(Input.GetAxis("Mouse ScrollWheel"));
        }

        // 角度とポジションはスワイプ中のみ更新処理が呼ばれるように(負荷改善)
        if(canMove || (IsEditor && !Application.isPlaying))
        {
    ...
    public void OnScreenSwipe()    => canMove = true;
    public void OnScreenEndSwipe() => canMove = false;
